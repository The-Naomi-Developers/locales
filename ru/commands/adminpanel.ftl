...
cmd-admin-panel-content-reaction_roles-description = Роли, выдаваемые пользователю при нажатии на эмодзи-реакцию у конкретного сообщения. Могут быть трех видов: **обычными** (выдаются при установке реакции, снимаются если убрать), **перманентными** (выдаются при установке реакции, остаются если убрать реакцию) и **переключаемыми** (это когда на одном сообщении может быть установлена только одна реакция, при установке второй - первая снимается).
